        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={step.title}
                className="relative bg-white border border-slate-200 rounded-2xl p-8 text-center hover:shadow-md transition-shadow"
              >
                <div className="w-14 h-14 mx-auto flex items-center justify-center rounded-full bg-brand-100 text-brand-600 mb-5">
                  <Icon className="w-7 h-7" />
                </div>
                <span className="absolute top-4 right-5 text-4xl font-extrabold text-slate-100">
                  {index + 1}
                </span>
